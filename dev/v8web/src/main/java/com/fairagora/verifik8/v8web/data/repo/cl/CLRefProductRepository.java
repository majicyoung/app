...
import com.fairagora.verifik8.v8web.data.domain.JTPondActivityProductTypes;
...
public interface CLRefProductRepository extends CodeListRepository<CLRefProduct> {
	...
	@Query(value = "SELECT cl_ref_products.*,jt_pond_activity_product_types.CL_FARM_POND_ACTIVITY_ID FROM cl_ref_products LEFT OUTER JOIN cl_ref_product_types ON cl_ref_products.CL_PRODUCTS_TYPE_ID = cl_ref_product_types.ID LEFT OUTER JOIN jt_pond_activity_product_types ON cl_ref_product_types.ID = jt_pond_activity_product_types.CL_PRODUCT_TYPE_ID WHERE cl_ref_products.ENABLED = TRUE ORDER BY cl_ref_products.NAME", nativeQuery = true)
	List<JTPondActivityProductTypes> getProductAndFarmPondActivityId();
}
